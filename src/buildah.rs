pub mod b {
    ...
    /// Returns buildah's graphroot from `buildah info`. The performance characteristics are pretty bad, .07s to execute, vs roughly .03 for 'buildah info'. Needed to reliably return find image layers and manifests.
    ...
    pub fn buildah_graph_root() -> Result<PathBuf, Box<dyn std::error::Error>> {
